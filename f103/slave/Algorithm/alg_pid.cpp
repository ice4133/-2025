...
void Gimbal::PID_Init(float kp,float ki,float kd,float integral_max,float output_max)
{
    this->Kp=kp;
    this->Ki=ki;
    this->Kd=kd;
...
//void pwm_correct(float* buffer)
//{
//	
//}
float Gimbal::PID_Compute(float error)
{
	this->pre_pre_error=this->last_error;
	this->last_error=this->error;
	this->error=error;
   ...
   if(this->integral > this->integral_max) 
	...
	float output=this->Kp*(this->error)
							+this->Ki*(this->integral)
							+this->Kd*(this->error-2*this->last_error+this->pre_pre_error);
